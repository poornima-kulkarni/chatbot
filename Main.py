...
mobile_mode = st.session_state.get('mobile_layout', False)

# Layout selection based on mobile mode
if mobile_mode:
    # Mobile layout - single column with stacked elements
    st.markdown('<div class="mobile-container">', unsafe_allow_html=True)
    
    # Mobile controls in expandable section
    with st.expander("📊 Chat Controls", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
            if st.button("📜 History", key="history_btn_mobile", help="Toggle chat history display"):
                toggle_history()
        
        with col2:
            if st.button("🗑️ Clear All", key="clear_mobile", help="Clear entire chat history"):
                clear_chat_history()
        ...
        # File uploader
        st.markdown('<div class="mobile-file-upload">', unsafe_allow_html=True)
        st.markdown("**📎 File Upload:**")
        uploaded_files = st.file_uploader(
            "Upload files:", 
            type=["png", "jpg", "jpeg", "pdf", "docx"], 
            accept_multiple_files=True,
            help="Upload images, PDFs, or Word documents"
        )
...
# Download functionality (works for both layouts)
...
if not mobile_mode:
    ...
    st.markdown(f"<div class='h4 mobile-prompt'><b>💬 Type your message:</b></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # Mobile layout toggle
    if st.button("📱 Mobile Layout", key="mobile_toggle", help="Toggle mobile-friendly layout"):
        st.session_state['mobile_layout'] = not st.session_state['mobile_layout']
        st.rerun()
    
    st.markdown(f"**Current Layout:** {'📱 Mobile' if mobile_mode else '💻 Desktop'}")
    ...
    st.markdown("""
    - **📱 Mobile Mode**: Better layout for small screens
    - **📎 Upload files**: Images, PDFs, and Word docs supported
    - **💾 Download**: Save conversations before refreshing
    - **🔒 Privacy**: Chats are temporary and not stored
    """)
